...
@cocotb.test()
async def design_pipeline(dut):
    #Generate the clock
    await cocotb.start(Clock(dut.clk, 10, 'us').start(start_high=False))
    clkedge = RisingEdge(dut.clk)


    dut.RESET_CONTROLLER.value = 1
    dut.pc_reset.value = 0
    ...
    await clkedge
    await clkedge
    await Timer(2, units="us") # MOV R1, #7
    assert dut.SYNTHESIZED_WIRE_75.value == 4 # pc mux0
    assert dut.SYNTHESIZED_WIRE_61.value == 0 # pc mux select
    assert dut.SYNTHESIZED_WIRE_0.value == 4 # pc mux out
    assert dut.SYNTHESIZED_WIRE_74.value == 0 # fetch pipeline out
    await Timer(2, units="us") # MOV R1, #7
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE3A01007 # inst


    await clkedge
    await Timer(2, units="us")  # MOV R3, #FFFF 
    assert dut.SYNTHESIZED_WIRE_75.value == 8 # pc mux0
    assert dut.SYNTHESIZED_WIRE_61.value == 0 # pc mux select
    assert dut.SYNTHESIZED_WIRE_0.value == 8 # pc mux out
    assert dut.SYNTHESIZED_WIRE_74.value == 4 # fetch pipeline out
    await Timer(2, units="us") # MOV R1, #7
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE3A030FF# inst

    await clkedge
    await Timer(2, units="us")  # MOV R7,1 
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE3A07001# inst

    await clkedge
    await Timer(2, units="us")  # MOV R4,0 
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE3A04000# inst

    await clkedge
    await Timer(2, units="us")  # SUB R3, R3, R1
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE0433001# inst

    await clkedge
    await Timer(2, units="us")  # MOV R5,0 
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE3A05000# inst

    await clkedge
    await Timer(2, units="us")  # MOV R6,0 
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE3A06000# inst

    await clkedge
    await Timer(2, units="us")  # MOV R6,0 
    assert dut.SYNTHESIZED_WIRE_29.value == 0xE0833007# inst

    await clkedge # decode
    await clkedge # execute
    await clkedge # execute

    assert dut.SYNTHESIZED_WIRE_8.value == 0XF9 # inst

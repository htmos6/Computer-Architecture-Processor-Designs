...
async def dp_lab2(dut):
    ...
    assert dut.SYNTHESIZED_WIRE_48.value == 0 # PC
    assert dut.inst.value == 0xE4121000
    ...
    await Timer(1, units="us")
    assert dut.SYNTHESIZED_WIRE_37.value == 7 # WRITE data 0
    ...
    assert dut.SYNTHESIZED_WIRE_48.value == 4 # PC
    ...
    await Timer(1, units="us")
    assert dut.SYNTHESIZED_WIRE_37.value == 5 # WRITE data 0

    await clkedge # ADD R2, R3, R1
    await Timer(1, units="us")
    assert dut.SYNTHESIZED_WIRE_1.value == 12 # PC'
    assert dut.SYNTHESIZED_WIRE_48.value == 8 # PC
    assert dut.inst.value == 0xE0832101
    await Timer(2, units="us")
    #assert dut.inst[15:12].value == 2 # WRITE Address 0
    assert dut.SYNTHESIZED_WIRE_35.value == 3 # READ 0 Reg File address
    assert dut.SYNTHESIZED_WIRE_36.value == 1 # READ 1 Reg File address

    await Timer(1, units="us")

    assert dut.SYNTHESIZED_WIRE_43.value == 5 # READ 0 Reg File
    assert dut.SYNTHESIZED_WIRE_44.value == 7 # READ 1 Reg File
    await Timer(1, units="us")
    assert dut.SYNTHESIZED_WIRE_37.value == 12 # WRITE Data
